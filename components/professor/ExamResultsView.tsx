...
export default function ExamResultsView({
  ...
  return (
    <div className="space-y-6">
      {/* Header & Exam Picker */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-2xl border border-zinc-800 bg-zinc-900/80 p-5">
        <div>
          <h3 className="text-lg font-bold text-white">Student Submissions & Integrity Audit</h3>
        ...
        <div className="flex items-center gap-3">
          <select
            ...
            className="rounded-xl border border-zinc-700 bg-zinc-950 px-3.5 py-2 text-xs sm:text-sm text-white focus:outline-none"
          >
            {exams.map((e) => (
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-xl bg-indigo-600 px-3.5 py-2 text-xs font-semibold text-white hover:bg-indigo-500 transition-colors shadow"
          >
      ...
      <div className="relative max-w-sm">
        ...
        <input
          ...
          className="w-full rounded-xl border border-zinc-800 bg-zinc-950 pl-10 pr-4 py-2.5 text-xs sm:text-sm text-white placeholder-zinc-500 focus:border-indigo-500 focus:outline-none"
        />
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs sm:text-sm text-zinc-300">
            ...
            <tbody className="divide-y divide-zinc-800/60">
              {filtered.map((s) => (
                <tr key={s.sessionId} className="hover:bg-zinc-800/40 transition-colors">
                  <td className="px-6 py-4">
                    <div className="font-bold text-white">{s.studentName}</div>
                    <div className="text-xs text-zinc-400 font-mono">{s.studentId} • {s.studentEmail}</div>
                  </td>
                  <td className="px-6 py-4">
                    <span
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`font-mono font-semibold ${
                        s.violations.length > 0 ? 'text-rose-400' : 'text-zinc-500'
                      }`}
                    >
                  ...
                  <td className="px-6 py-4 text-right">
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-1.5 text-xs font-semibold text-zinc-200 hover:text-white hover:bg-zinc-700 transition-colors"
                    >
      ...
      {activeSessionForAudit && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="relative w-full max-w-2xl rounded-2xl border border-zinc-800 bg-zinc-900 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            {/* Header */}
            ...
            <div className="p-6 max-h-[60vh] overflow-y-auto space-y-4">
              <div className="grid grid-cols-2 gap-4 text-xs bg-zinc-950 p-3.5 rounded-xl border border-zinc-800">
                <div>
                ...
                <div>
                  ...
                  <span className="text-white font-semibold capitalize font-mono">
              ...
              <div>
                ...
                {activeSessionForAudit.violations.length > 0 ? (
                  <div className="space-y-2">
                    {activeSessionForAudit.violations.map((viol, index) => (
                      <div
                        key={viol.id}
                        className="rounded-xl border border-zinc-800 bg-zinc-950 p-3.5 flex items-start gap-3"
                      >
                        <div
                  ...
                  <div className="p-8 text-center rounded-xl bg-zinc-950 border border-zinc-800 text-emerald-400 text-xs">
            ...
            {/* Footer */}
            <div className="border-t border-zinc-800 bg-zinc-950/80 px-6 py-4 flex justify-end">
              <button
                onClick={() => setActiveSessionForAudit(null)}
